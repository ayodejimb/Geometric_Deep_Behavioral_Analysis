...
all_dist = {'within': [30,569], 'beyond': [30,1803], 'combine': [30,1892]}  # epoch and LSTM parameters

def load_data(cat):
    file_path_data = r'C:\Downloads\MATLAB_DATA_FOLDERS\2_MONTHS\LOOCV_no_obj\{}\all_data.npy'.format(cat)  # for 4 or 12 months run, change 2 to 4 or 12; for OiP, use its path 
    file_path_label = r'C:\Downloads\MATLAB_DATA_FOLDERS\2_MONTHS\LOOCV_no_obj\{}\all_label.pickle'.format(cat)
...
for opt_, data in all_dist.items():    
    ...
    save_att = r'C:\Desktop\with_attention\both_attn_results_OR\2M\{}'.format(opt_)
    os.makedirs(save_att, exist_ok=True)
    file_path = r"C:\Downloads\MATLAB_DATA_FOLDERS\2_MONTHS\{}\classsification_attn_results.json".format(opt_)
            ...
            for r in range(runs):
                ...
                for epoch in range(training_epochs):
                    ...
                    for i in range(steps + (1 if len(X_train) % batch_size != 0 else 0)):                       
                        ...
                        y_pred_sigmoid = torch.round(torch.sigmoid(output.data)).squeeze(1)
                        total += labels.size(0)
                        correct += (y_pred_sigmoid == labels.long()).sum().item()
                ...
                with torch.no_grad():
                    ...
                    for i in range(steps):                        
                        ...
                        total_test += 1
                        correct_test += (y_pred_sigmoid == labels.long()).sum().item()
