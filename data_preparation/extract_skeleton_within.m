function extract_skeleton_within(Project_Path)

% **** This function is for <=76px (<=2cm) ******
...
for v = Idx2use
    ...
    % Let's create a table to store those frames that satisfies the distance condition
    varTypes = [];
    for i = 1:size(DLC_Data,2)
        varTypes{i} = 'double';
    end

    % Now, let us create names for the columns (previous column names + the ones for the object)
    ...
    % We need the normalized object coordinates to be in this format - (1*18 columns)
    ...
    for i = 1:size(DLC_Data,1)
        ...
        % Nose coordianates for each frame
        Nose = DLC_Data{i,2:3};
    
        % Euclidean distance for the old object and new object boundaries (pixel coords)
        dist_ = pdist2(Nose, old_obj_coords');
        dist_2 = pdist2(Nose, new_obj_coords');
        ...
        if any(dist_ <= 76)
            ...
            explore_old = explore_old + 1;
          
        elseif any(dist_2 <= 76)
             frame_data = norm_coords{i,:};

             frame_data = array2table(frame_data, 'VariableNames', column_names); % Convert to table to be able to add in the main one
             explore_new = explore_new + 1;
